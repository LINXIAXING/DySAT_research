...
class StructuralAttentionLayer(nn.Module):
    ...
    def forward(self, graph):
        ...
        edge_index = graph.edge_index  # [2, 66]
        edge_weight = graph.edge_weight.reshape(-1, 1)
        H, C = self.n_heads, self.out_dim
        # 计算e_uv时其中的W*X操作
        x = self.lin(graph.x).view(-1, H, C) # [N, heads, out_dim] N个node,heads个头,每个头有8维的信息
        # attention
        # 
        alpha_l = (x * self.att_l).sum(dim=-1).squeeze() # [N, heads]
        alpha_r = (x * self.att_r).sum(dim=-1).squeeze()
        alpha_l = alpha_l[edge_index[0]]  # [num_edges, heads] 出发节点的attention
        alpha_r = alpha_r[edge_index[1]]  # 终止节点的attention值
        # 拼接,乘上边权重
